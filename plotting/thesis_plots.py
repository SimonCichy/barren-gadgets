...
def training_plots_with_statistics(): 
    ...
    fig = formatter.figure(aspect_ratio=1, wide=True)
    plt.rcParams.update({
        "pgf.preamble": "\n".join([
            r"\usepackage{dsfont}", 
    ...
    for l, lamb in enumerate(lambdas):
        ...
        lgd = axs[1].legend(handles=custom_lines,
                        ...
                        borderpad=0)
    plt.tight_layout()
    plt.savefig(data_folder + '../plots/training_new_gadget/trainings_for_thesis.pdf', 
...
def variances_plots():
    ...
    fig = formatter.figure(aspect_ratio=.7, wide=False)
    plt.rcParams.update({
        "pgf.preamble": "\n".join([
            r"\usepackage{dsfont}", 
            r"\usepackage{amsmath}", 
