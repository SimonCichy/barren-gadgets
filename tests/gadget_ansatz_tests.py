...
def test3():
    ...
    cost_cat = qml.ExpvalCost(hea.ansatz, Pcat, dev_gad)
    cost_gs_comp = qml.ExpvalCost(hea.ansatz, Pground_comp, dev_gad)
    cost_gs_anc = qml.ExpvalCost(hea.ansatz, Pground_anc, dev_gad)
...
def test7():
    ...
    @qml.qnode(dev_gad)
    def circuit(params):
        hea.ansatz(params, wires=range(total_qubits))
        return qml.state()
